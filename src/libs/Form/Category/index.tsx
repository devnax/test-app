...
const Category = ({ name, form, defaultValue, items, onChange, hideSearch, onCreate, perpage }: CategoryProps) => {
   const chunks = useMemo(() => chunkArray(items.filter((cat) => !cat.parentId), perpage || 20), [items, perpage])
            ...
            <FormGroup>
               {
                  (chunks[page - 1] || []).map((category) => {
                     ...
                     return <Fragment key={category.value}>
                        <FormControlLabel
                           control={<Checkbox
                              size="small"
                              sx={{ p: .4 }}
                        ...
                        />
                        {
                           isChecked && items.filter((cat) => category.value === cat.parentId).map((subcat) => {
...

            {
               onCreate && <>
                  ...
                  <Box sx={{ textAlign: "right" }}>
                     ...
                     >{createParent ? 'PARENT - ' + items.find((c) => c.value === createParent)?.label : 'SELECT PARENT'}</Link>}
                  </Box>
               </>
            }
         </Box>
